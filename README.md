...
- 생성 이미지: `img/prompts.json`(지시문·스타일) → `XAI_API_KEY=… python3 tools/gen_images.py` → `img/wall/{dawn,day,dusk,night}.webp`(시간대별 배경, 있으면 자동 적용) · `img/covers/NN.webp`(논문 표지 썸네일, 있으면 목록에 표시). 방문 시 생성 없음, 파일만 읽음.
- 사진: `img/profile.jpg`
